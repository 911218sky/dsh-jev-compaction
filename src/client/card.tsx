...
/** Structural settings form (0.1.7 ConfigForm; formerly SettingsScope). */
interface SettingsFormLike<T> {
  getSnapshot(): {
    status: string;
    value: T | undefined;
    base: unknown;
...
export interface JevCompactionCardFace {
  readonly scope: SettingsFormLike<JevCompactionConfig>;
}
...
type ScopeOps = Parameters<SettingsFormLike<JevCompactionConfig>["mutate"]>[0];

const PROVIDER_OPTIONS = [
  { value: "openai", label: "OpenAI-compatible chat (any gateway)" },
  { value: "typesafe", label: "TypeSafe Jev (hosted System One)" },
  { value: "jeff", label: "Self-hosted System One-compatible server" },
  { value: "custom", label: "Custom System One endpoint" },
] as const;
