...
export interface JevCompactionConfig {
   ...
   */
  readonly decision?: {
    /** Decision backend: openai (chat/completions) or System One (typesafe/jeff/custom). */
    readonly provider?: SystemOneProvider;
    readonly openai?: SystemOneProviderOverride;
...
export const JevCompactionConfigSchema = z.object({
  ...
  decision: z.object({
    provider: z
      ...
      .description(
        "Decision backend: openai (OpenAI-compatible chat), typesafe, jeff, or custom System One.",
      ),
    openai: z
      .object({
        baseUrl: z
          .string()
          .description("OpenAI-compatible API base (…/v1)."),
        apiKeyEnv: z
          .string()
